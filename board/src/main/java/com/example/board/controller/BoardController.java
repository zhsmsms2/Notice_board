...
public class BoardController extends UiUtils {
	...
	public String openBoardWrite(@ModelAttribute("params") BoardDTO params, @RequestParam(value = "n_idx", required = false) Long n_idx, Model model) {
		if (n_idx == null) {
			model.addAttribute("board", new BoardDTO(n_idx, null, null, null, null, null, null, null));
		} else {
			BoardDTO board = boardService.getBoardDetail(n_idx);
	...
	public String openBoardDetail(@ModelAttribute("params") BoardDTO params, @RequestParam(value = "n_idx", required = false) Long n_idx, Model model) {
		if (n_idx == null) {
	...
	public String deleteBoard(@ModelAttribute("params") BoardDTO params, @RequestParam(value = "n_idx", required = false) Long n_idx, Model model) {
		if (n_idx == null) {
			return showMessageWithRedirect("올바르지 않은 접근입니다.", "/board/list.do", Method.GET, null, model);
		}
		...
		Map<String, Object> pagingParams = getPagingParams(params);
		try {
				boolean isDeleted = boardService.deleteBoard(n_idx);
				if (isDeleted == false) {
